...
from python_parser.run_parser import extract_dataflow, get_identifiers
...
def main():
    ...
    success_attack = 0
    total_cnt = 0
    attacker = MHM_Attacker(args, model, codebert_mlm, tokenizer, token2id, id2token)
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            _res = attacker.mcmc_random(tokenizer, code,_label=true_label, _n_candi=30, _max_iter=100, _prob_threshold=1, subs=substitutes)
            attack_type = "mhm"
