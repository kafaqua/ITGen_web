    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            if len(identifiers) == 0:
                continue
            if len(substitutes.keys())*50 > len(id2token):
                selected_tmp_sub = random.sample(id2token, len(id2token))
                m = len(id2token)//len(substitutes.keys())
                sublists = [selected_tmp_sub[i:i+m] for i in range(0, len(selected_tmp_sub), m)]
            else:
