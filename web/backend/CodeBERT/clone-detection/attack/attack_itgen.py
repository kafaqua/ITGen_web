...
def main():
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            # 计算需要的样本数量
            required_samples = len(substitutes.keys()) * 50
            available_tokens = len(id2token)
            
            # 如果需要的样本数超过可用 token 数，使用所有可用的 token，然后重复采样
            if required_samples > available_tokens:
                # 先使用所有可用的 token
                selected_tmp_sub = list(id2token)
                # 如果需要更多，则重复随机采样（允许重复）
                additional_needed = required_samples - available_tokens
                additional_samples = random.choices(list(id2token), k=additional_needed)
                selected_tmp_sub.extend(additional_samples)
            else:
                selected_tmp_sub = random.sample(id2token, required_samples)
