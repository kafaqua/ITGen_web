...
from utils import Recorder, set_seed, get_code_tokens, build_vocab
...
def main():
    ...
    parser.add_argument('--block_num', type=int, default=4) #5
    parser.add_argument('--batch_size', type=int, default=4)
    parser.add_argument('--update_step', type=int, default=1)
    parser.add_argument('--max_patience', type=int, default=50) # 50
    ...
    query_times = 0
    # uid_pattern = re.compile(r'^[a-zA-Z]+$')
    uid_pattern = re.compile(r'^[a-zA-Z_][a-zA-Z0-9_]*$')

    Indexs = []
    with open('/home/yanmeng/huangli/Attack/CodeT5/Clone-detection/attack/result/attack_bayes_all.jsonl') as f:
        for line in f:
            js = json.loads(line.strip())
            if js['Adversarial Code'] == None and js['Index'] < 2001:
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            if len(identifiers) == 0:
                continue
            selected_tmp_sub = random.sample(id2token, len(substitutes.keys())*30)
            sublists = [selected_tmp_sub[i:i+50] for i in range(0, len(selected_tmp_sub), 30)]
            tmp_sub = []
            for sub in sublists:
                tmp = []
                for s in sub:
            ...
            adv_code, is_success, replaced_words = attacker.bayes_attack(example, selected_sub, code_pair, query_times, logits, example_start_time)
            attack_type = "bayes"
