...
from utils import CodeDataset, select_parents, crossover, map_chromesome, mutate, _tokenize, get_identifier_posistions_from_code, \
    get_masked_code_by_position, isUID, is_valid_variable_name, _tokenize, get_replaced_var_code_with_meaningless_char
...
def get_statement_identifier(first_idx, identifiers):
    file_path = "/data/hl/Attack/dataset/Vulnerability-prediction/test.csv"
...
class WIR_Attacker():
    ...
    def filter_identifier(self, code, identifiers):
        try:
            _, code_tokens = get_identifiers(remove_comments_and_docstrings(code, "python"), "python")
        except:
            _, code_tokens = get_identifiers(code, "python")
        filter_identifiers = []
        for identifier in identifiers:
            if is_valid_variable_name(identifier, lang='python'):
                position = []
...
class RNNS_Attacker():
    ...
    def _get_variable_info(self):
        ...
        for code in codes:
            ...
            # cur_variables = self.filter_identifier(code, cur_variables)
...
class Beam_Attacker():
    ...
    def is_vaild(self, code_token, identifier):
        if not is_valid_variable_name(identifier, lang='python'):
            return False
        position = []
    ...
    def perturb(self, example, code, all_substitues, tgt_word, equal=False):
        ...
        cosine_list = []
        for sub in all_substitues:
            temp_code = get_example(code, tgt_word, sub, 'python')
        ...
        for substitute in select_substitues:
            if not is_valid_variable_name(substitute.strip(), lang='python'):
                continue
            substitute_list.append(substitute.strip())
            temp_replace = get_example(final_code, tgt_word, substitute.strip(), 'python')
        ...
        for index, temp_prob in enumerate(logits):
            ...
            if temp_label != orig_label:
                ...
                gaps.append(
                    [is_success, get_example(final_code, tgt_word, candidate, 'python'), candidate, temp_prob[temp_label]])
    ...
    def beam_attack(self, orig_prob, example, substitutes, code, statement_dict, beam_size):
        ...
        tmp_code = code
        try:
            _, code_token = get_identifiers(remove_comments_and_docstrings(tmp_code, "python"), "python")
        except:
        ...
        for i_iter in range(len(adv_identifier)):
            ...
            for replace_info, value in final_pop_copy.items():
                ...
                for seq in value["sequence"]:
                    try:
                        _, code_token = get_identifiers(remove_comments_and_docstrings(value["adv_code"], "python"), "python")
                    except:
                        print("syntax errors!")
                        continue
                    if not self.is_vaild(code_token, seq):
